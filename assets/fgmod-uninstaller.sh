...
# === Remove files tracked by the installer ===
tracked_restore=()
python_cmd="${PYTHON:-python}"
if ! command -v "$python_cmd" >/dev/null 2>&1 && command -v python3 >/dev/null 2>&1; then
  python_cmd="python3"
fi
if [[ -f "FRAMEGEN_PATCH_FILES" ]]; then
  echo " Removing tracked Framegen payload files..."
  while IFS=$'\t' read -r tracked_kind tracked_path; do
    [[ -n "$tracked_kind" && -n "$tracked_path" ]] || continue
    [[ "$tracked_path" == */* || "$tracked_path" == *".."* ]] && continue
    if [[ "$tracked_kind" == "file" ]]; then
      rm -f "$tracked_path"
      [[ -f "$tracked_path.b" ]] && tracked_restore+=("$tracked_path")
    elif [[ "$tracked_kind" == "dir" ]]; then
      rm -rf "$tracked_path"
    fi
  done < <("$python_cmd" - <<'PY' 2>/dev/null || true
import json
try:
    with open("FRAMEGEN_PATCH_FILES", "r", encoding="utf-8") as f:
        data = json.load(f)
    for name in data.get("files", []):
        if isinstance(name, str):
            print(f"file\t{name}")
    for name in data.get("dirs", []):
        if isinstance(name, str):
            print(f"dir\t{name}")
except Exception:
    pass
...
rm -f "fakenvapi.dll" "fakenvapi.ini"
...
# === Remove D3D12_Optiscaler directory ===
...
# === Restore tracked payload backups ===
for restored_file in "${tracked_restore[@]}"; do
  if [[ -f "$restored_file.b" ]]; then
    mv -f "$restored_file.b" "$restored_file"
    echo " Restored original $restored_file"
    logger -t fgmod-uninstaller "Restored original $restored_file"
  fi
done
unset tracked_restore restored_file python_cmd
